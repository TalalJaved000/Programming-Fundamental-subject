...
/*
 * Purpose   : save all medicines from the arrays into medicines.txt
 *             Each line has:  name price stock
 *             Deleted entries (blank name) are skipped so the file stays clean.
 * Parameters: none
 * Returns   : void
 */
void saveToFile()
{
    ofstream file(FILE_NAME); // open file for writing (overwrites old content)
...
/*
 * Purpose   : load medicines from medicines.txt into the arrays at startup.
 *             If the file does not exist yet, load the hard-coded defaults
 *             and immediately save them so the file is created.
 * Parameters: none
 * Returns   : void
 */
void loadFromFile()
{
    ...
    while (file >> medName[medCount] >> medPrice[medCount] >> medStock[medCount])
    {
        ...
        if (medCount >= MAX_SIZE)
            break; // safety: stop if array is full
    }
...
void deleteMedicine()
{
    ...
    // soft-delete: blank out the entry
    medName[idx]  = "";
    medPrice[idx] = 0;
    medStock[idx] = 0;
...
void runPharmacistMenu()
{
    while (true)
    {
        ...
        if      (option == 1) showAllMedicines();
        else if (option == 2) addMedicine();
        else if (option == 3) searchMedicine();
        else if (option == 4) updateMedicine();
        else if (option == 5) deleteMedicine();
...
void buyMedicine()
{
    ...
    saveToFile(); // save updated stock to file after each purchase
...
void runCustomerMenu()
{
    ...
    while (true)
    {
        ...
        if      (option == 1) showAvailableMedicines();
        else if (option == 2) buyMedicine();
        else if (option == 3) printBill();
...
/*
 * Purpose   : load inventory from the file.
 *             If the file is missing (first run), use hard-coded defaults
 *             and save them to create the file automatically.
 */
void initMedicines()
{
...
int main()
{
    initMedicines(); // load from file (or defaults on first run)
    ...
    while (true)
    {
        ...
        if      (choice == 1) runPharmacistLogin();
        else if (choice == 2) runCustomerMenu();
